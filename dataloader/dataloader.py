...
class Bcgdataset2(Dataset):
    '''读取数据集，返回acf之前的数据，acf之后的数据'''

    def __init__(self, root,acf_pick):
        super(Bcgdataset2, self).__init__()
        #   root 数据根目录
        self.root = root
    ...
    def __getitem__(self, pig_info):
        ...
        if self.acf_pick == 1:
            # print('use_acf')
            data_bcg_noacf = torch.FloatTensor(data_bcg).unsqueeze(1)
            data_bcg = np.array(data_bcg)
            for i in range(len(data_bcg)):
                xi = data_bcg[i]
                xi = cal_acf(xi)
                data_bcg[i] = xi
        normalize = transforms.Normalize(mean=[0], std=[1])
        data_bcg = torch.FloatTensor(data_bcg).unsqueeze(1)
        data_bcg = normalize(data_bcg)
        data_bcg_noacf = normalize(data_bcg_noacf)
        return torch.cat((data_bcg,data_bcg_noacf),dim=-2), torch.LongTensor(label.numpy()),torch.LongTensor(domain_label.numpy())
class Bcgdataset(Dataset):
    '''读取数据集'''

    def __init__(self, root,acf_pick=1,dual_channel=0):
    ...
    def __getitem__(self, pig_info):
        ...
        data_bcg = torch.FloatTensor(data_bcg).unsqueeze(1)
        data_bcg = normalize(data_bcg)
        if self.dual_channel == 1:
            data_bcg_origin =  torch.FloatTensor(data_bcg_origin).unsqueeze(1)
            data_bcg_origin = normalize(data_bcg_origin)
            data_bcg = torch.cat([data_bcg,data_bcg_origin],dim=1)
        return data_bcg, torch.LongTensor(label.numpy()),torch.LongTensor(domain_label.numpy())
def cal_acf(x):
    """ 计算自相关序列 """
    x_unbiased = x - x.mean()
...
def get_K_fold_crossvalidation_data_11FOLD(dataset, pigset, K=11, i=0):
    '''K折交叉验证之第i折'''
    assert K > 1, '没有1折交叉验证望周知'
    print(f'{K}折交叉验证之第{i}折开始')
    X_train, y_train, domain_y_train = None, None, None
    ...
    print(len_pigset)
    # random.shuffle(pigset)
    slice_size = len_pigset // K # 将数字“向上取整”,23 // 6 = 3.xxx = 4
    print('slice_size', slice_size)
    print(pigset)
    for j in range(K):
        # 最后一折剩下的全拿走
        if j == i:  # 0-5
            idx = slice(j * slice_size, (j + 1) * slice_size if j != K - 1 else len_pigset)
            pig_idx_set = pigset[idx]
            print('验证集', pig_idx_set)
            # print('K-1idx', idx)
            # print('pig', pig_idx_set)
...
# def get_K_fold_crossvalidation_data2(dataset, pigset, K=6, i=0):
#     '''K折交叉验证之第i折,还返回了没有acf的数据'''
#     assert K > 1, '没有1折交叉验证望周知'
#     print(f'{K}折交叉验证之第{i}折开始')
#     X_train, X_train_noacf, y_train, domain_y_train = None, None, None, None
#     X_Valid, y_valid, domain_y_valid = None, None, None
#     len_pigset = len(pigset)  # 23
#     print(len_pigset)
#     # random.shuffle(pigset)
...
#             # print('pig', pig_idx_set)
#             for pig_idx in pig_idx_set:
#                 x_part, _, y_part,domain_y_part = dataset[pig_idx]
#                 if X_Valid is None:
#                     X_Valid, y_valid, domain_y_valid = x_part, y_part, domain_y_part
#                 else:
#                     X_Valid = torch.cat([X_Valid, x_part], 0)
...
#             print('训练集',pig_idx_set)
#             for pig_idx in pig_idx_set:
#                 x_part, x_part_noacf, y_part,domain_y_part = dataset[pig_idx]
#                 if X_train is None:
#                     X_train, X_train_noacf, y_train, domain_y_train = x_part, x_part_noacf, y_part,domain_y_part
#                 else:
#                     X_train = torch.cat([X_train, x_part], 0)
#                     X_train_noacf = torch.cat([X_train_noacf, x_part_noacf], 0)
#                     y_train = torch.cat([y_train, y_part], 0)
#                     domain_y_train = torch.cat([domain_y_train,domain_y_part],0)
#     return X_train,X_train_noacf, y_train,domain_y_train, X_Valid, y_valid,domain_y_valid
...
def data_generator2(train_features, train_labels,domain_train_labels, test_features, test_labels,domain_test_labels,batch_size):
    print(f'训练集维度{train_features.shape}，测试集维度{test_features.shape}\n'
          f'[样本数，1，样本长度]，875=125*7,7s')
    count_train = torch.bincount(train_labels)
    count_test = torch.bincount(test_labels)
    ...
    test_len = len(test_labels)
    train_iter = load_array((train_features, train_labels,domain_train_labels), batch_size, is_train=True)
    test_iter = load_array((test_features[:,0:1,:], test_labels,domain_test_labels), batch_size, is_train=False)
    return train_iter,test_iter,train_len,


def collate_cbow(batch,vocab_size,window):
    """
    ...
    for text_tokens_ids in batch:
        text_tokens_ids = text_tokens_ids[0].reshape(-1,vocab_size)
        seq_len = text_tokens_ids.shape[0]
        if len(text_tokens_ids) < window * 2 + 1:
            continue
        for idx in range(seq_len):
            center = text_tokens_ids[idx, :].unsqueeze(0)
            if idx < window:
                context_idx = list(range(idx)) + list(range(idx + 1, 2*window + 1))
            elif idx >= window and idx < seq_len - window:
                context_idx = list(range(idx - window, idx)) + list(range(idx + 1, idx + window + 1))
            else:
                context_idx = list(range(seq_len-2*window-1, idx)) + list(range(idx + 1, seq_len))
            context = torch.cat([text_tokens_ids[i, :].unsqueeze(0) for i in context_idx], dim=0)
            batch_input.append(center)
            batch_output.append(context)
    batch_input = torch.stack(batch_input)
    batch_output = torch.stack(batch_output)
    return batch_output,batch_input
def load_array_word2vec(data_arrays, batch_size, is_train=True,vocab_size=25,window=2):
    """Construct a PyTorch data iterator.

    Defined in :numref:`sec_linear_concise`"""
    dataset = data.TensorDataset(data_arrays)
    if is_train:
        return data.DataLoader(dataset, batch_size=batch_size,drop_last=True, collate_fn=partial(collate_cbow,vocab_size=vocab_size,window=window))
    else:
        return data.DataLoader(dataset, batch_size=batch_size,shuffle=is_train,collate_fn=partial(collate_cbow,vocab_size=vocab_size,window=window))
def data_generator_WORD2VEC(train_features, train_labels,domain_train_labels, test_features, test_labels,domain_test_labels,batch_size,vocab_size=25,window=2):
    print(f'训练集维度{train_features.shape}，测试集维度{test_features.shape}\n'
          f'[样本数，1，样本长度]，875=125*7,7s')
    count_train = torch.bincount(train_labels)
    count_test = torch.bincount(test_labels)
    ...
    train_iter = load_array((train_features, train_labels,domain_train_labels), batch_size, is_train=True)
    test_iter = load_array((test_features, test_labels,domain_test_labels), batch_size, is_train=False)
    train_iter_word2vec = load_array_word2vec(train_features, batch_size, is_train=True,vocab_size=vocab_size,window=window)
    test_iter_word2vec = load_array_word2vec(test_features, batch_size, is_train=False,vocab_size=vocab_size,window=window)
    return train_iter,test_iter,train_len,test_len,train_iter_word2vec,test_iter_word2vec
