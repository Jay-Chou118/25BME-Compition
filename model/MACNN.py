...
class ThreeClassClassifier(nn.Module):
    def __init__(self, input_size, hidden_size):
        super(ThreeClassClassifier, self).__init()
        self.binary_01 = BinaryClassifier(input_size, hidden_size)
        self.binary_12 = BinaryClassifier(input_size, hidden_size)

    def forward(self, x):
        binary_01_output = self.binary_01(x)
        binary_12_output = self.binary_12(x)

        # 计算标签0, 1, 2
        label_0 = binary_01_output
        label_2 = binary_12_output
        label_1 = 1 - label_0 - label_2

        return torch.cat((label_0, label_1, label_2), dim=1)