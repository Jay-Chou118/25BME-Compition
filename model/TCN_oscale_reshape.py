...
def get_Prime_number_in_a_range(start, end):
    ...
    return Prime_list
def TemporalConvNet(c_in, layers, ks=2, dropout=0.,conv_small=32,prime_list=None):
    temp_layers = []
...
class TCN(nn.Module):
    def __init__(self, c_in, c_out, layers=8*[30], ks=7, conv_dropout=0., fc_dropout=0.,conv_small=32):
        super(TCN, self).__init__()
        receptive_field_shape = 875 // conv_small
        prime_list = get_Prime_number_in_a_range(1, receptive_field_shape)
        layers = len(layers) * [3 * len(prime_list)]
        self.tcn = TemporalConvNet(c_in, layers, ks=ks, dropout=conv_dropout, conv_small=conv_small,prime_list=prime_list)
        self.gap = GAP1d()
