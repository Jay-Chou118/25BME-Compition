...
def get_Prime_number_in_a_range(start, end):
    ...
    return Prime_list
def TemporalConvNet(c_in, layers, ks=2, dropout=0.,conv_small=32,seq_len=875):
    temp_layers = []
    receptive_field_shape = seq_len//conv_small
    ...
    for i in range(len(layers)):
        ...
        if i >= 1:
            ni = nf//len(prime_list)*len(prime_list)
        nf = layers[i]
    ...
    return nn.Sequential(*temp_layers), ni
...
class TCN(nn.Module):
    def __init__(self, c_in, c_out,vocab_size,embed_size,first_model=None, layers=8*[30], ks=7, conv_dropout=0., fc_dropout=0.,conv_small=32):
        super(TCN, self).__init__()
        self.vocab_size = vocab_size
        self.embed_size = embed_size
        self.firstlayer = first_model.embeddings
        self.seq_len = 875//vocab_size*embed_size
        self.tcn,ni = TemporalConvNet(c_in, layers, ks=ks, dropout=conv_dropout, conv_small=conv_small,seq_len=self.seq_len)
        self.gap = GAP1d()
        self.dropout = nn.Dropout(fc_dropout) if fc_dropout else None
        self.linear = nn.Linear(ni,c_out)
        self.init_weights()
    ...
    def forward(self, x):
        x = x.reshape(x.shape[0],-1,self.vocab_size)
        x = self.firstlayer(x)
    ...
    def freeze_first_model(self):
        for param in self.firstlayer.parameters():
