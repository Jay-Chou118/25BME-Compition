...
from trainer.training_evaluation import model_evaluate2,cross_grad_train
import random

import numpy as np
device_cuda = 2
config = Config()
def cross_grad_Trainer(model,train_dl,test_dl,device,train_len,test_len):
    train_ls, test_ls = [], []
    ...
    for epoch in range(config.num_epoch):
        ...
        test_loss, test_acc, VF_test_acc, SR_test_acc, MA_test_acc = model_evaluate2(label_classifier,test_dl,criterion,device,test_len)
...
def k_fold_normal(model, k, dataset, device, pig_train_set):
    ...
    for i in range(k):
        ...
        train_ls, valid_ls, train_acc, valid_acc, train_VF_acc_set, train_SR_acc_set, train_MA_acc_set, valid_VF_acc_set, valid_SR_acc_set, valid_MA_acc_set \
            = cross_grad_Trainer(model,train_iter,test_iter,device,train_len,test_len)
